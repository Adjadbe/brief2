    ...
    if (isset($recherche)) {
        if (($lieu != 0) && ($annee != 0)) {
            for ($mois = 1; $mois <= 12; $mois++) { 
                $info = $conn->prepare("SELECT COUNT(*) AS nbmeurtres_atteintes_physiques FROM meurtres_atteintes_physiques INNER JOIN ville ON lieu = id WHERE MONTH(dateEvenement) = $mois AND (lieu =$lieu) OR (YEAR(dateEvenement) = $annee)");
                $info->execute();
                $info->setFetchMode(PDO::FETCH_ASSOC);
                $tab = $info->fetch();
                $total[] = $tab['nbmeurtres_atteintes_physiques'];
            }
        }
    }
        ...
        <div class="secondaire"> <!-- fond degrade -->
            <div class="trois">
                <div class="sidebar_gche">
                    <div class="men">
                          <a href="index.php"style="margin-bottom:50px"> <div class="puce_blanche"> <p style="color:#3e3f5f; margin-left: 12px; margin-top: 10px;">01</p> </div> <div class="list-item">Accident</div></a>
                            <hr>
                ...
                </div> <div class="logo"> <img src="logo2.png" alt="logo" width="80px" ></div>
