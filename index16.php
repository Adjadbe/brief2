    ...
    if (isset($recherche)) {
        if (($lieu != 0) && ($annee != 0)) {
            for ($mois = 1; $mois <= 12; $mois++) { 
                $info = $conn->prepare("SELECT COUNT(*) AS nbinondation FROM inondations INNER JOIN ville ON lieu = id WHERE MONTH(dateEvenement) = $mois AND (lieu =$lieu) OR (YEAR(dateEvenement) = $annee)");
                $info->execute();
                $info->setFetchMode(PDO::FETCH_ASSOC);
                $tab = $info->fetch();
                $total[] = $tab['nbinondation'];
            }
        }
    }
